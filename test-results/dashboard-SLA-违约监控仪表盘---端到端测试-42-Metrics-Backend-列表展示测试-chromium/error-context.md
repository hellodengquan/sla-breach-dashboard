...
- Name: dashboard.spec.js >> SLA 违约监控仪表盘 - 端到端测试 >> 42. Metrics Backend 列表展示测试
- Location: tests/e2e/dashboard.spec.js:618:3
  ...
  - generic [ref=e3]:
    - banner [ref=e4]:
      - generic [ref=e5]:
        ...
        - paragraph [ref=e7]:
          - text: "实时监控客服工单 SLA 超时情况 · 更新于 2026/6/15 14:32:25 · 当前用户:"
          - strong [ref=e8]: 张三
          - generic [ref=e9]: (管理员)
      - generic [ref=e10]:
...
> 625 |     await expect(page.getByText('Prometheus')).toBeVisible();
  ...
  716 |     await page.waitForTimeout(500);
  717 | 
  718 |     const editButton = page.getByRole('button', { name: '✏️ 编辑视图' });
  719 |     await editButton.click();
  720 |     await page.waitForTimeout(300);
  721 | 
  722 |     await expect(page.locator('.widget-picker')).toBeVisible();
  723 |     await expect(page.getByText('选择要显示的组件')).toBeVisible();
  724 | 
  725 |     const doneButton = page.getByRole('button', { name: '✓ 完成编辑' });
```